...
function loadCanvas() {
	...
	if (document.getElementById('lowRes').checked) {rSF = 1}
	else {rSF = 2};
...
function numToNote(num) {
	...
	if (showFlats) {noteLetRef = [,"G#","A","A#","B","C","C#","D","D#","E","F","F#","G"]}
	
	if (Array.isArray(num)) {numList = num}
	else {return noteLetRef[num]}
...
function createScale() {
	...
	if (tonicNote == 0) {
		...
		*/
		...
		switch (mode) {
			case 1: scaleFormula = [2,2,1,2,2,2,1]; colorCode=["#6cb7e6",,,,,,"#b4d7ed"]; break;	// Ionian
			case 2: scaleFormula = [2,1,2,2,2,1,2]; colorCode=["#6cb7e6",,,,,"#b4d7ed",]; break;	// Dorian
			case 3: scaleFormula = [1,2,2,2,1,2,2]; colorCode=["#6cb7e6","#b4d7ed",,,,,]; break;	// Phrygian
			case 4: scaleFormula = [2,2,2,1,2,2,1]; colorCode=["#6cb7e6",,,"#b4d7ed",,,]; break;	// Lydian
			case 5: scaleFormula = [2,2,1,2,2,1,2]; colorCode=["#6cb7e6",,,,,"#b4d7ed",]; break;	// Mixolydian
			case 6: scaleFormula = [2,1,2,2,1,2,2]; colorCode=["#6cb7e6",,,,,,"#b4d7ed"]; break;	// Aeolian
			case 7: scaleFormula = [1,2,2,1,2,2,2]; colorCode=["#6cb7e6",,,,,,]; break;	// Locrian
			case 8: scaleFormula = [2,2,3,2,2]; colorCode=["#6cb7e6",,,,"#b4d7ed"]; break;			// Major Pentatonic
			case 9: scaleFormula = [3,2,2,3,3]; colorCode=["#6cb7e6",,,,"#b4d7ed"]; break;			// Minor Pentatonic
			case 10: scaleFormula = [2,3,2,3,3]; colorCode=["#6cb7e6",,,,"#b4d7ed"]; break;			// Egyptian Pentatonic
			case 11: scaleFormula = [2,3,2,2,3]; colorCode=["#6cb7e6",,,,"#b4d7ed"]; break;			// Blues Major Pentatonic
			case 12: scaleFormula = [3,2,3,2,2]; colorCode=["#6cb7e6",,,,"#b4d7ed"]; break;			// Blues Minor Pentatonic
		}
	...
	function mapNotes(nList, cList, showSharp) {
		...
		function findIndexes(value, array) {
			...
			for (let i=0; i<array.length; i++) {
				if (array[i] == value) {indexList.push(i)}
			}
			return indexList;
		}
		...
		for (let i = 1; i <= 7; i++) {
			if (document.getElementById("chord" + i + "toggle").checked) {
				transArray = chordNotes(activeNotesNum[i-1], document.getElementById("chord" + i + "type").innerHTML);
		...
		for (let noteIt=0; noteIt <= nList.length; noteIt++) {
			...
			for (let stringIt=0; stringIt < stringOpenNotes.length; stringIt++) {
				...
				let indx = findIndexes(nList[noteIt], generateStringNotes(stringOpenNotes[stringIt]));
				// Creates note on canvas for each time the note is found on the string searched
				for (let mapIt = 0; mapIt < indx.length; mapIt++) {
					let transparency = "ff";
					if (!transArray.includes(nList[noteIt])) {transparency = "44"}
					
					createNote(indx[mapIt], stringIt + 1, numToNote(nList[noteIt]), cList[noteIt], transparency);
...
function updateScaleNotesDiv() {
	...
	if (tonicNote == 0) {document.getElementById("scaleNotes").style.display = "none"}
	else {
...
function chordNotes(chordNum, chordType) {
	...
	for (let i=0; i<retArray.length; i++) {
		if (retArray[i] > 12) {retArray[i] = retArray[i] - 12}
	}
	return retArray;
}
...
function updateChordSection() {
	function displayMajor(chordNum, numeral) {
		...
		document.getElementById("chord" + chordNum + "name").innerHTML = numToNote(activeNotesNum[chordNum - 1]);
